      ...
      <nav className="backdrop-blur-md bg-white/70 dark:bg-gray-900/30 text-gray-900 dark:text-white border-b border-gray-200 dark:border-white/10 shadow-lg">
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-1">
            {links.map((link) => (
              <Link key={link.to} to={link.to} className={`px-4 py-2 rounded-lg text-sm font-medium transition-all backdrop-blur-sm ${pathname === link.to ? 'bg-gray-900/10 dark:bg-white/30 text-gray-900 dark:text-white shadow-md' : 'hover:bg-gray-900/5 dark:hover:bg-white/10 text-gray-700 dark:text-white/90'}`}>
                {link.label}
              </Link>
            ))}
            <LanguageSelector />
        ...
        {open && (
          <div className="md:hidden backdrop-blur-md bg-white/70 dark:bg-gray-900/30 border-t border-gray-200 dark:border-white/20 px-4 pb-4 space-y-1">
            {links.map((link) => (
              <Link key={link.to} to={link.to} onClick={() => setOpen(false)} className="block px-4 py-2 rounded-lg text-sm text-gray-700 dark:text-white hover:bg-gray-900/10 dark:hover:bg-white/20 transition-colors">
                {link.label}
              </Link>
            ))}
          </div>
